...
| CI | `.github/workflows/ci.yml` | The `npm run check` chain on every push and PR, plus the stale-types guard (fails if CI's own typegen produces a diff against the committed types). | Deployment. The live site auto-deploys from `master` through the Cloudflare dashboard, wired separately. |
