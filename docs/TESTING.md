...
| CI                   | `.github/workflows/ci.yml`                    | On every push to `main`/`staging` and every PR: install, the canonical-file drift gate, typegen + the stale-types guard, astro check, lint, format check, unit tests, build, Studio build, link check; and in a parallel job the Playwright suites with the report uploaded. | Deployment. `main` auto-deploys through Cloudflare Workers Builds; `staging` deploys via `deploy-staging.yml`.                        |
