...
| -------------------- | ----------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| Type check           | `npx astro check`                               | Every `.astro`, `.ts` and `.tsx` under the site. `tsconfig.json` excludes `modules/` and `dist` only. Since the studio folded into this package (2026-09-06) the Studio config, schemas and custom components ARE type-checked here, which is how the `@sanity/ui` v4 `space` -> `gap` rename was found. | Studio **runtime** errors: schema mistakes pass the check and crash in the browser.                                     |
| Lint                 | `npm run lint`                                  | eslint over `src`, `scripts`, `tests` and `playwright.config.ts`.                                                                                                                                                                                                                                        | Formatting (below).                                                                                                     |
| Format               | `npm run format:check`                          | prettier with the astro and tailwind plugins, family `.prettierrc`. `.prettierignore` lists what stays hand-formatted (generated files, staged modules, the PORTABLE starter files, prose archives). The Studio is no longer exempt.                                                                     | Files in `.prettierignore`.                                                                                             |
...
| Type + schema check  | `npm run typegen`                               | Regenerates `schema.json` and `src/lib/sanity.types.ts` from `src/sanity/schemaTypes/`.                                                                                                                                                                                                                  | Nothing about the deployed Studio: the embedded `/studio` is built by `astro build`, so it cannot be behind the schema. |
...
| Canonical drift      | `npm run sync-check`                            | Every file marked `PORTABLE: canonical copy` byte-compared against `ncs-astro-sanity-starter`. A hard CI gate: 0 drifted or the build is red. Compares against the starter's `main`, or its `staging` for a staging build.                                                                               | Files without the marker. A deliberate fork drops the marker and says so in its header.                                 |
| Preview stack        | `npx wrangler dev -c dist/server/wrangler.json` | That the SSR routes answer. Expect `/` 200, `/studio` 200, and `/api/draft-mode/enable` **401 "Invalid preview secret"** when a token is present, or **503** naming what is missing when it is not. A 500 there means a real failure, not a fail-closed one.                                             | The signed-in Studio and the Presentation panel, which need a browser and a real editor session.                        |
...
| Older full sweep     | `npm run check:full`                            | typegen, build, unit tests in one go (the pre-standard `check`).                                                                                                                                                                                                                                         | Browser suites, links, Lighthouse.                                                                                      |
...
| Canonical-file drift | `npm run sync-check`                            | That this repo's copies of the shared family files still match `ncs-astro-sanity-starter`. A CI gate since 2026-09-06, not only a hand-run check.                                                                                                                                                        | Files without the PORTABLE header, which includes `scripts/page-parity.mjs` (ported as a pattern on purpose).           |
| CI                   | `.github/workflows/ci.yml`                      | On every push to `main`/`staging` and every PR: install, the canonical-file drift gate, typegen + the stale-types guard, astro check, lint, format check, unit tests, build (site + embedded Studio), link check; and in a parallel job the Playwright suites with the report uploaded.                  | Deployment. `main` auto-deploys through Cloudflare Workers Builds; `staging` deploys via `deploy-staging.yml`.          |
