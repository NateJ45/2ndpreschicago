...
- `HeroArchSlideshow.astro` -- the same cross-fading Ken Burns slideshow, but sized for the home page's arched photo frame (no full-bleed darkening overlay, since it is a framed standalone image rather than a backdrop for text). Reuses the global `.hero-slideshow` / `.hero-slide` CSS and the advance script from `HeroBackground.astro`. Used by `index.astro` when the Home singleton's `heroImages` holds 2+ images and no seasonal hero is active; the parent frame must be `position:relative`.
