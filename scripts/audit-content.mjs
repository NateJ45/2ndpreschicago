...
async function run() {
  ...
  for (const t of SINGLETONS) {
    ...
    if (pub) {
      console.log(
        `  ${t.padEnd(16)} published — ${contentFieldCount(pub)} content fields set${draft ? ' (+ unpublished draft)' : ''}`,
      );
    } else if (draft) {
      draftOnly++;
  ...
  if (!staff.length) console.log('  (none loaded)');
    ...
    staff.forEach((s) =>
      console.log(
        `  - ${s.name ?? '(no name)'}${s.role ? ` — ${s.role}` : ''}${s.photo ? '' : '  [no photo]'}`,
      ),
    );
  ...
  console.log(
    ss ? JSON.stringify(ss, null, 2) : '  MISSING — header/footer run on site.ts fallbacks',
  );

  console.log(`\nSummary: ${missing} singleton(s) missing, ${draftOnly} draft-only.`);
  console.log(
    'Note: the live static site reads PUBLISHED content; draft-only + missing docs render from code fallbacks.\n',
  );
}
