...
const SEEDS = {
  ...
  foodPage:        { keyword: 'no questions asked', defaultHeadline: 'Food for all in need, no questions asked' },
  useOurSpacePage: { keyword: 'space',              defaultHeadline: 'Interested in using space at Second?' },
  weddingsPage:    { keyword: 'Landmark',           defaultHeadline: 'Get married in a National Historic Landmark' },
...
for (const [id, { keyword, defaultHeadline }] of Object.entries(SEEDS)) {
  const doc = byId[id];
  if (!doc) { console.log(`SKIP ${id}: no published document`); continue; }
  if (doc.heroKeyword) { console.log(`SKIP ${id}: heroKeyword already set ("${doc.heroKeyword}")`); continue; }
  const headline = doc.heroHeadline || defaultHeadline;
  if (!headline.includes(keyword)) {
    console.log(`SKIP ${id}: "${keyword}" not in current headline "${headline}"`);
    continue;
  }
  console.log(`${APPLY ? 'SET ' : 'WOULD SET'} ${id}.heroKeyword = "${keyword}"  (headline: "${headline}")`);
  if (APPLY) {
