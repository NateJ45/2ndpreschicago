#!/usr/bin/env node
// PORTED from ncs-astro-sanity-starter scripts/page-parity.mjs (2026-08-27).
//
// Deliberately does NOT carry the PORTABLE canonical marker. PORTS.md card 3
// records that the parity harness travels as a PATTERN, not as a byte-identical
// file: the WCP session had to add repo-specific normalizer rules, and any repo
// may need its own. A marked copy would report DRIFT for an adaptation that is
// correct, which trains people to ignore the drift check.
//
// This copy is unmodified from the starter apart from this header. If you add a
// normalizer rule, put a dated comment beside it saying why, and consider whether
// the rule generalizes - if it does, port it back to the starter and add the
// PORTS.md card in the same commit.
/**
