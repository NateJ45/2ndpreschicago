...
async function uploadImage(file) {
  ...
  const asset = await client.assets.upload('image', readFileSync(resolve(assetsDir, file)), {
    filename: file,
  });
...
const IMAGES = [
  ...
  },
  {
    ...
    alt: 'The painted choir of angels on the sanctuary ceiling that gives the church its nickname',
  },
  {
    id: 'aboutPage',
    field: 'buildingImage',
    file: 'sanctuary-interior.webp',
    alt: 'The nave of Second Presbyterian, lined with Tiffany windows beneath the Arts and Crafts ceiling',
  },
  {
    id: 'worshipPage',
    field: 'childrenImage',
    file: 'tiffany-windows.webp',
    alt: 'Three Tiffany landscape windows of trees and hills in the sanctuary',
  },
  {
    id: 'musicPage',
    field: 'organImage',
    file: 'mural-angels.jpg',
    alt: "A painted panel of angels with instruments in the sanctuary's Arts and Crafts decoration",
  },
];
...
async function run() {
  console.log(
    `Seeding section images + officeHours to ${projectId}/${dataset} (setIfMissing)...\n`,
  );
  for (const it of IMAGES) {
    const assetId = await uploadImage(it.file);
    await patchSetIfMissing(
      it.id,
      { [it.field]: img(assetId, it.alt) },
      `${it.id}.${it.field} (${it.file})`,
    );
    await patchSetIfMissing(
      `drafts.${it.id}`,
      { [it.field]: img(assetId, it.alt) },
      `drafts.${it.id}.${it.field}`,
    );
  }
