...
test.describe('Focus indicators are visible in dark mode', () => {
  for (const route of formRoutes) {
    test(`${route} gives every field a visible focus ring in dark mode`, async ({
      ...
      // native picker owns focus there), so every select comes back with no
      // ring while every text field passes, on the SAME focus classes
      // (reid-design-site staging CI, 2026-09-05). Chromium exercises the real
      // styling for all of them; the WebKit result is the harness, not the site.
