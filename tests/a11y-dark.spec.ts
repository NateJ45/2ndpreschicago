...
// ('secondpres-theme') and toggles the `dark` CLASS on <html>; it does NOT
...
//
// The forms on this site are Sanity-driven (FormRenderer), so the routes that
// carry one are read from the build in routes.ts rather than listed here.
// =============================================================================
...
test.describe('Focus indicators are visible in dark mode', () => {
  for (const route of formRoutes) {
    test(`${route} gives every field a visible focus ring in dark mode`, async ({ page }) => {
      ...
      // the class list, the selects on the Sanity forms had no focus indicator
      // at all on Safari and iOS. WebKit is exactly where this bug lives, so
      // it is exactly where the check has to run.
      await forceDark(page);
      await page.goto(route);
      await expect(page.locator('html')).toHaveClass(/dark/);
      await settle(page);
      ...
      for (let i = 0; i < count; i++) {
        ...
        // shadow alone even with its focus ring gone; only a shadow that focus
        // CHANGES is a focus indicator.
